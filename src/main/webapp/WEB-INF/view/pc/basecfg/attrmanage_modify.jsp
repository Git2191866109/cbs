...
//	initSelect('isFixed','PC.Common.Boolean',"dfinput");
	...
	$("input[name=isFixed]").initCol({tagClass:"dfinput",dataSrc:"dictionary",dataCode:"PC.Common.Boolean",dataCond:"",async:"false",
		onload:function (){
			$.thisObj = $("select[name=isFixed]");
			var isFixed = $.thisObj.val();
			if(isFixed == 1) {
				$("input[name=code]").attr("readonly","readonly");
			}else {
				$("input[name=code]").removeAttribute("");
			}
		},
		onchange:function (){
...
<body>
<form:form method="post" action="${base}/form/submit/attribute/modifyComplete" commandName="attribute" name="attribute" >
