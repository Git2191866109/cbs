...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".tabitem").click(function(){
			...
			$(".tabcont").hide();
			var src = $("#" + id + "_iframe").attr("src");
			if(src == null) {
				$("#" + id + "_iframe").attr("src",$("#" + id + "_iframe").attr("url"));
			}
			$("#"+id).show();
		});
		$(".tabitem").first().click();
...
<ul id="tabnav" class="tabnav">
	<shiro:hasPermission name="${pc_productmanager_product_newplayerproduct_code}">
...
<shiro:hasPermission name="${pc_productmanager_product_newplayerproduct_code}">
	<c:forEach items="${categoryList }" var="item">
		<div class="tabcont" id="tabcont_${item.value }" style="">
			<iframe  id="tabcont_${item.value }_iframe"  url="${base}/goto/product/productItemIndex?viewCode=${pc_productmanager_product_newplayerproduct_code}_TAB${item.value }&isContextCode=1&categoryId=${item.value }" width="100%" frameborder="0" width="100%" name="iframe" class="iframe" scrolling="auto" style="" title="iframe" ></iframe>
		</div>
	</c:forEach>
</shiro:hasPermission>
</body>
