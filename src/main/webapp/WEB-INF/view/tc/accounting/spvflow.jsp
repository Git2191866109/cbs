...
<head>
    ...
    $(document).ready(function () {
      jQuery("#accountingDetailList").jqGrid({
        ...
        colModel: [
          ...
          {name: 'corporationName', index: 'corporationName', width: '15%', align: 'center', hidden: false, sortable: false},
  ...
  function search() {
      ...
      jQuery("#accountingDetailList").jqGrid("setGridParam", {
       ...
       url:"${base}/ajax/getdata/spvAccountingDetail/selectSPVPaginated.json"
      }).trigger("reloadGrid");
      var userData = $("#accountingDetailList").jqGrid("getGridParam", "userData");
	  ...
	  $("#t_accountingDetailList").append(amount);
	  
	  $.ajax({
			url:'${base}/ajax/getdata/spvAccountingDetail/searchSpvStaticesFlow.json',
			...
			success:function(data,status){
					if(status == "success"){
						$("#totalFlowOut").text(data.totalFlowIn);
						$("#totalFlowIn").text(data.totalFlowOut);
    ...
    <td width="100%">
      <table cellpadding="0" height="85" cellspacing="0" border="1" width="100%">
        <tr>
         ...
         <td>
            <label for="flowTo" style="float:left;line-height: 32px;">资金流向：</label>

            <div class="vocation">
              <select name="flowTo" id="flowTo" class="select3">
        ...
        <tr>
          ...
          <td>
            <label for="accountType" style="float:left;line-height: 32px;">财务类型：</label>
        ...
        <tr>
         <td width="5%"></td>
          ...
          <td>
            <label for="spvId" style="float:left;line-height: 32px;">SPV 公司名称：</label>
            <div class="vocation">
              <select name="spvId" id="spvId" class="select3">
              			<c:forEach var="spv" items="${spvList}">
