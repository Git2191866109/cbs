...
$(document).ready(function(){
    jQuery("#${config.modelNameSuffix?uncap_first}List").jqGrid({
        url: '<#noparse>${</#noparse>base<#noparse>}</#noparse>/ajax/getdata/${config.modelNameSuffix?uncap_first}/treegrid.json',
        datatype: 'json',
        colNames: [
		<#if (fileds?size >0) >	
		...
		<#if shortcutList.menuType?? && shortcutList.menuType?contains('shortcut')>
    	    ...
    	    $(".shortcut_${shortcutList.viewType!""}").click(function(){
    	    	var rowid = $(this).attr("id");
    			window.location.href="<#noparse>${</#noparse>base<#noparse>}</#noparse>/goto/${config.modelNameSuffix?uncap_first}/gopage/${shortcutList.viewName!""}?id="+rowid;
    	    });    
    	    </#if>
    	    </shiro:hasPermission>
	    </#if>
		...
		<#if shortcutList.menuType?? && shortcutList.menuType?contains('top')>
		    ...
		    $("#top_${shortcutList.viewType!""}","#t_${config.modelNameSuffix?uncap_first}List").click(function(){
		    	window.location.href = "<#noparse>${</#noparse>base<#noparse>}</#noparse>/goto/${config.modelNameSuffix?uncap_first}/gopage/${shortcutList.viewName!""}";
		    });    
		    </shiro:hasPermission>			
		</#if>
		</#list> 	
