...
<body>
<form:form method="post" action="<#noparse>${</#noparse>base<#noparse>}</#noparse>/form/submit/${config.modelNameSuffix?uncap_first}/modify/<#noparse>${</#noparse>${config.codeKey}_code<#noparse>}</#noparse>" commandName="${config.modelNameSuffix?uncap_first}" name="${config.modelNameSuffix?uncap_first}">
<#if (fileds?size >0) >	
<#list fileds as list>
<#if list.hidden == '1'>
